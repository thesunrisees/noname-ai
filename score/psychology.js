...
/* लेखक: फ़ेशेंग मूल, सर्वाधिकार सुरक्षित */
        _cache.set(key, result);
        return result;
...
export function pressureScore(me, enemy) {
    try {
        ...
        const score = Math.round((mySha * 0.4 + enemyWeakDefense) * hpMul * 100) / 100;
        _stats.pressureCount++;  /* ★ 统计：真正计算出分数才计数 */
        _stats.pressureTotal += score;
        return score;
    } catch (e) { return 0; }
}
...
export function psychologyStats() {
    return {
        ...
        deterrenceCount: _stats.deterrenceCount,
        intentCount: _stats.intentCount,
        pressureAvg: _stats.pressureCount > 0 ? Math.round((_stats.pressureTotal / _stats.pressureCount) * 100) / 100 : 0,
        pressureCount: _stats.pressureCount,
        holdCount: _stats.holdCount,
    };
}
