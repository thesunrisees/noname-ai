...
function threatOf(tgt) {
    ...
    /* ★ 调整：忠臣对主公的仇恨值降低 */
    try {
        if (typeof getCamp === 'function') {
            const att = getCamp(from, to);
            /* 如果是同阵营（忠臣-主公），仇恨值减半 */
            if (att > 0) {
                result = result * 0.5;
            }
...
/* ★ 带目标阵营判断的卡牌价值（延时锦囊/控制牌使用时调用） */
export function cardValueWithTarget(card, me, target) {
	try {
		let v = cardValueOf(card, me);
		if (!target) return v;
		
		/* 导入敌我判断和阵营调整系数 */
		const { CARD_ALIGN_MOD } = require('./value-tables.js');
		const id = cardIdOf(card);
		
		/* 检查这张牌有没有阵营调整系数 */
		const mod = CARD_ALIGN_MOD[id];
		if (!mod) return v;
		
		/* 判断目标是敌人还是队友 */
		const isEnemy = isEnemyOf(target, me);
		if (isEnemy) {
			v += mod.enemy;  /* 贴敌人加分 */
		} else {
			v += mod.ally;   /* 贴队友减分 */
...
function targetScore(me, tgt) {
	try {
		...
		/* ★ 人类玩家额外加分：AI优先打人类玩家，增加难度（温和版） */
		try {
			const isHuman = !tgt.isAI && !tgt.npc;
			if (isHuman) s += 1.0;  /* 人类玩家 +1.0 分，温和加成 */
		} catch (e) {}
		
		/* ★ 改成加法叠加，减少波动 */
		s += canReachOf(me, tgt) ? 1.0 : -2.0; // 距离：能打到+1，打不到-2
		try { s += (tgt.countCards ? tgt.countCards("h") : 0) * 0.1; } catch (e) {} // 手牌：每张+0.1
		if (tgt.hp !== undefined && tgt.hp <= 2) s += 0.5; // 残血：+0.5（容易收头）
		try { if (hasBadStatus(tgt)) s -= 0.5; } catch (e) {} // 负面状态：-0.5
		if (tgt.hp !== undefined && tgt.hp === tgt.maxHp && tgt.countCards && tgt.countCards("h") < 2) s -= 0.3;
		...
		try {
			...
			if (tgt.getEquip && tgt.getEquip('zhuge')) s += 1.0;
			/* 3. 血量缺口：满血目标威胁更大，残血目标威胁小 */
			if (tgt.hp !== undefined && tgt.maxHp !== undefined) {
				const hpRatio = tgt.hp / tgt.maxHp;
				if (hpRatio >= 0.8) s += 0.3;   /* 满血 → +0.3 */
				else if (hpRatio <= 0.3) s -= 0.5;  /* 残血 → -0.5 */
			}
			/* 4. 技能威胁：有强输出技能的目标威胁大 */
			if (tgt.hasSkill) {
