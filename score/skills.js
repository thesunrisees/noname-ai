...
export function renderSkillBreakdownText(sid) {
	try {
		...
		if (br.isPureAI) {
			extras.push('');
			extras.push('⚠ 该技能为 AI 辅助类，不改变游戏状态');
		}
		if (br.isRuleOnly) {
			extras.push('');
			extras.push('⚙ 该技能为规则约束类，不产生直接攻防效果');
		}
		if (br.isUnrecognized) {
			extras.push('');
			extras.push('❓ 该技能未被引擎识别');
			extras.push('  可能原因：效果在嵌套子技能 / 动态调用 / 关联技能间接生效');
		}
		...
		extras.push('');
		extras.push('【识别来源】' + (br.source || 'empty'));
