...
import { VAL_CARD, VAL_EFFECT, VAL_TIMING, STRATEGY, SKILL_RULE } from '../misc/value-tables.js';
import { codeGainOf, scanCharacters, buildAutoSkillRules, charComboOf, advice, detectCombo, clearGainCache, skillTagsOf, aggregateSkillTags, skillProfileOf, skillProfitBreakdown, renderSkillBreakdownText } from '../skill/skills.js';
import { renderScoreBreakdown } from '../skill/skillRules.js';
import { enemiesOf, isEnemyOf, threatOf, situationFactor, cardValueOf, seatPressure, distancePressure, mountValue, forecastSummary, incomingPressure } from '../strategy/threat.js';
import { miniPredict } from '../core/mini-model.js';
import { miniFeatures } from '../skill/skills.js';
import { getRound, getScoreLog, getREC, resetRound, getMEM, bestAction, modelDecision, isGameOver, getDecisionLog, clearDecisionLog, appendDecision } from '../core/engine.js';
import { trainExportAndDownload, trainClearBuffer, trainStats } from '../export/trainExport.js';
import { cfg } from '../core/util.js';
import { getObs, explainObs, relationOf, observedStance, hostilityOf, friendlinessOf, styleOf } from '../strategy/observer.js';
import { identityOf, confidenceOf, beliefOf, explainIdentity, updateBelief, currentMode } from '../identity/identity.js';
import { teamPlan } from '../strategy/team.js';
import { resourceBalance, sellHpValue, equipStripRisk } from '../economy/economy.js';
import { storeStats, resetStore, importStore, clearStore, exportStore } from '../strategy/memory.js';
...
import { getArchive, archiveStats, clearArchive, fmtTime, getGameDecisions, exportArchiveJson, fullArchiveStats } from '../export/archive.js';
import { exportAll, exportAllJson, exporters, downloadFile, exportAllFile, exportModuleFile, exportBatchFiles, downloadTextFile } from '../export/export.js';
import { exportPersonality, importPersonality, copyToClipboard } from '../misc/share.js';
import { getFeedbackStats, resetFeedback, feedbackCount } from '../feedback/feedback.js';
import { deckAutoDetect, deckSetMode, deckGetMode, totalRemaining, suitRemaining, cardRemaining, deckSnapshot, deckInitialCounts, deckAvailableModes } from '../card/deckMemory.js';
import { lebuEscapeRate, bingliangEscapeRate, shandianHitRate, baguaSuccessRate, expectDrawValue } from '../card/deckPredict.js';
import { getTagConf, resetStyleFeedback } from '../feedback/styleFeedback.js';
import { multiTurnForecast } from '../misc/multiturn.js';
import { PERSONALITY_TEMPLATES, findTemplate, listCustomTemplates, saveCustomTemplate, deleteCustomTemplate, allTemplates, findTemplateAll } from '../misc/templates.js';
import { lineChart, barChart, radarChart, donutChart } from './charts.js';
import { aggregateByPlayer, buildComparisonHtml } from '../analysis/aiStats.js';
import { t, setLang, availableLangs } from '../misc/i18n.js';
import { startAutoplay, stopAutoplay, autoplayStatus, showAutoplayReport, initAutoplayMonitor, startBatchAutoplay, stopBatchAutoplay, batchStatus, showBatchReport } from '../misc/autoplay.js';
import { exportDecisionsJson, exportDecisionsMarkdown, parseDecisionsJson, showImportedDecisions } from './report.js';
import { importArchiveJson } from '../export/archive.js';
import { getPlayerMemoryStats, resetPlayerMemory } from '../feedback/styleFeedback.js';
import { skillBranchesOf, checkBranch, skillStagesOf, skillInteractionOf } from '../skill/skills.js';
import { listProfiles, saveProfile, loadProfile, deleteProfile, renameProfile, exportProfilesJson, importProfilesJson, resetProfiles, profileCount, buildSnapshot, applySnapshot } from '../misc/profiles.js';
import { getLogLevel, setLogLevel } from '../core/logger.js';
import { selfCheck } from '../core/compat.js';
import { getDecisionFeedbackStats, resetDecisionFeedback } from '../feedback/decisionFeedback.js';
import { perfStats, perfHistory, perfReset } from '../misc/perf.js';
import { openSmartPanel } from './smartPanel.js';
import { listRecommends, applyRecommend, snapshotCurrent } from '../misc/recommend.js';
import { healthCheck } from '../selfcheck/health.js';
import { recentExplains } from '../misc/explain.js';
import { adaptiveStatus } from '../learning/adaptive.js';
import { recommendChars } from '../misc/pickRecommend.js';
...
function openHealthPanel() {
	try {
		let content = '=== 引擎健康度 ===\n\n';
		try {
			const sc = window.__DJSC.selfCheck();
...
function openFeedbackPanel() {
	try {
		let content = '=== 决策回放 ===\n\n';
		try {
			const log = window.__DJSC.decisionLog();
...
function openArchivePanel() {
	try {
		let content = '=== 战报归档 ===\n\n';
		try {
			const archive = window.__DJSC.archive();
...
function openConfigPanel() {
	try {
		...
		/* 标题栏 */
		html += "<div style='font-size:20px;color:#00FFB0;margin:20px;text-align:center;font-weight:bold;padding:10px;background:#14243c;border-radius:8px;'>⚙️ 当前配置</div>";

		/* 数据管理按钮区 */
		html += "<div style='margin:20px;padding:16px;background:#14243c;border-radius:12px;'>";
		html += "<b style='color:#9ad8ff;font-size:16px;'>📦 数据管理</b>";
		html += "<div style='margin-top:12px;display:flex;flex-direction:column;gap:10px;'>";
		/* 导出按钮 */
		html += "<button id='djsc-btn-export-all' style='padding:14px 20px;font-size:16px;background:linear-gradient(135deg,#1e90ff,#00bfff);color:white;border:none;border-radius:10px;cursor:pointer;box-shadow:0 4px 12px rgba(30,144,255,0.3);'>📥 导出所有面板数据</button>";
		/* 导入按钮（覆盖） */
		html += "<button id='djsc-btn-import-overwrite' style='padding:14px 20px;font-size:16px;background:linear-gradient(135deg,#ff6b6b,#ee5a5a);color:white;border:none;border-radius:10px;cursor:pointer;box-shadow:0 4px 12px rgba(255,107,107,0.3);'>📤 导入数据（覆盖现有）</button>";
		/* 导入按钮（不覆盖） */
		html += "<button id='djsc-btn-import-merge' style='padding:14px 20px;font-size:16px;background:linear-gradient(135deg,#ffa500,#ff8c00);color:white;border:none;border-radius:10px;cursor:pointer;box-shadow:0 4px 12px rgba(255,165,0,0.3);'>📤 导入数据（不覆盖，只追加）</button>";
		html += "</div>";
		html += "<div style='margin-top:10px;font-size:11px;color:#888;'>· 导出：把所有面板数据打包成 JSON 下载<br>· 导入（覆盖）：清空现有数据，全部替换<br>· 导入（不覆盖）：只导入新数据，不覆盖现有</div>";
		html += "</div>";

		/* 配置内容区 */
		html += "<div style='margin:20px;padding:16px;background:#14243c;border-radius:12px;'>";
		try {
			html += "<b style='color:#9ad8ff;'>--- 核心开关 ---</b><br>";
			html += "决策积分引擎：" + (cfg('decisionScore', true) ? '✅ 开启' : '❌ 关闭') + '<br>';
			html += "决策模式：" + (cfg('mode', 'mix') || 'mix') + '<br><br>';

			html += "<b style='color:#9ad8ff;'>--- 倾向设置 ---</b><br>";
			html += "进攻倾向：" + (cfg('atkBias', 1) || 1) + '<br>';
			html += "防守倾向：" + (cfg('defBias', 1) || 1) + '<br><br>';

			html += "<b style='color:#9ad8ff;'>--- AI 增强 ---</b><br>";
			html += "战术规划器：" + (cfg('enablePlanner', true) ? '✅ 开启' : '❌ 关闭') + '<br>';
			html += "决策反馈：" + (cfg('decisionFeedback', true) ? '✅ 开启' : '❌ 关闭') + '<br>';
			html += "响应AI：" + (cfg('responseAI', true) ? '✅ 开启' : '❌ 关闭') + '<br>';
			html += "广播协作：" + (cfg('broadcastAI', true) ? '✅ 开启' : '❌ 关闭') + '<br>';
			html += "拼点AI：" + (cfg('compareAI', true) ? '✅ 开启' : '❌ 关闭') + '<br><br>';

			html += "<b style='color:#9ad8ff;'>--- 性格设置 ---</b><br>";
			html += "性格预设：" + (cfg('riskProfile', 'custom') || 'custom') + '<br>';
			html += "攻守轴：" + (cfg('personalityAggression', 50) || 50) + '<br>';
			html += "冒险轴：" + (cfg('personalityRisk', 50) || 50) + '<br>';
			html += "团队轴：" + (cfg('personalityTeam', 50) || 50) + '<br>';
		} catch (e) {
			html += '获取配置失败：' + e.message + '<br>';
		}
		html += "</div>";
...
function openMemoryPanel() {
	try {
		let content = '=== 跨局记忆 ===\n\n';
		try {
			const memory = window.__DJSC.memory();
...
function openSkillPanel() {
	try {
		let content = '=== 技能矩阵 ===\n\n';
		try {
			const matrix = window.__DJSC.matrixOf();
...
/* ================= 导出 ================= */
export { openScorePanel, installDebugBridge, uninstallDebugBridge, startDialogGuard, stopDialogGuard };
