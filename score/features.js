...
function qScore(score) {
    if (score === undefined || score === null) return 64;  /* 默认中间值 */
    if (score <= 0) return 0;
    if (score >= 10) return 127;
    return (score / 10 * 127) | 0;
}
...
export function extractFeatures(me, act, ctx, out, allPlayers) {
    ...
    /* ★ 改成用卡牌分数，而不是固定开关 */
    f[2] = _has(me, 'sha') ? qScore(window.__DJSC.autoLearn ? window.__DJSC.autoLearn.getCardScore('sha') : 5) : 0;
    f[3] = _has(me, 'shan') ? qScore(window.__DJSC.autoLearn ? window.__DJSC.autoLearn.getCardScore('shan') : 5) : 0;
    f[4] = _has(me, 'tao') ? qScore(window.__DJSC.autoLearn ? window.__DJSC.autoLearn.getCardScore('tao') : 5) : 0;
    f[5] = _has(me, 'wuxie') ? qScore(window.__DJSC.autoLearn ? window.__DJSC.autoLearn.getCardScore('wuxie') : 5) : 0;
...
/* Penulis: Feisheng Original, Semua hak terpelihara */
