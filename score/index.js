...
export function installScoreEngine() {
	 ...
	 */
	try {
		import('./core/index.js').then(function (m) {
			window.__DJSC = window.__DJSC || {};
			window.__DJSC.modular = {
				isEnabled: m.isScoreEngineEnabled,
				install: m.installScoreEngine,
				uninstall: m.uninstallScoreEngine,
			};
			window.__DJSC.modularReady = true;
			log.info('init', '子目录模块化版已加载（仅注册，不重复安装）');
		}).catch(function (e) {
			log.warn('init', '子目录模块化版加载失败: ' + (e && e.message ? e.message : e));
		});
	} catch (e) {}
	...
	try {
		const MODULES_TO_EXPOSE = [
			...
			['decision', './analysis/decisionRegistry.js'],
			['bandit', './bandit.js'],
