...
import { cacheGet, cacheSet, checkStateChanged, initStateWatcher } from './cache.js';
...
import { autoRegister as globalAutoRegister, installProbes } from './globalScanner.js';
import './missingModules.js';  // ★ 缺失模块补全：5个真正工作的模块
import './autoLearn.js';       // ★ 自动学习表：新卡牌/新技能自动打分
/* ===== 24 个优化模块 ===== */
import { probHasWuxie, probHasTao, probHasSha, probHasJiu, probHasCard, inferHand } from './handInference.js';
import { aggregateByPlayer, buildComparisonHtml as aiStatsBuildHtml } from './aiStats.js';  /* ★ 真正的 aiStats */
import { checkAllyExempt } from './allyExempt.js';  /* ★ 真正的 allyExempt */
import { recordGuardEvent, getGuardStats, resetGuardRecorder } from './guardRecorder.js';  /* ★ 真正的 guardRecorder */
import { healthCheck } from './health.js';  /* ★ 真正的 health */
import { trainLocalAsync, isTraining, lastResult } from './localTrainer.js';  /* ★ 真正的 localTrainer */
import { getDecisionFeedbackStats } from './decisionFeedback.js';  /* ★ 真正的 decisionHook（新增） */
import { installAutoDiscover, promoteHighRegretPoints as promoteRegret, getRegretStats } from './autoDiscover.js';  /* ★ 真正的 decisionRegistry */
import { scanCharacters, aggregateSkillTags, buildAutoSkillRules } from './skills.js';  /* ★ 真正的 skillScanner（新增） */
import { wuxieTiming, wuxieBonus } from './wuxieTiming.js';  /* ★ 真正的 responseAI */
import { wuguTiming, wuguBonus } from './wuguTiming.js';  /* ★ 真正的 responseAI */
import { archiveStats } from './archive.js';  /* ★ 真正的 replayAnalysis（新增） */
import { exportStore } from './memory.js';  /* ★ 真正的 profiles（新增） */
import { autoSelfHeal } from './selfHeal.js';
import { updateAdaptive } from './adaptive.js';
import { showReport, resetReportShown } from './report.js';
import { resetDecisionFeedback } from './decisionFeedback.js';
import { recordGameEnd as banditRecordGameEnd } from './bandit.js';
import { onGameEnd as modelStateOnGameEnd, recordABScore } from './modelState.js';
...
import { getFeedbackStats } from './feedback.js';  /* ★ 真正的 skillCustom（新增） */
import { detectConflict, conflictLog, conflictStats, resetConflict } from './conflictDetector.js';  /* ★ 真正的 compat */
import { getStyleFeedbackStats, getPlayerTag } from './styleFeedback.js';  /* ★ 真正的 smartPanel（新增） */
import { getStats as strategistGetStats, getLastDecision, setEnabled, isEnabled } from './strategist.js';  /* ★ 真正的 decisionDashboard（新增） */
import { focusTarget, protectScore, comboWithAllies } from './team.js';  /* ★ 真正的 modules（新增） */
import { openSelfCheck, getSelfCheckHistory, clearSelfCheckHistory } from './selfCheck.js';  /* ★ 真正的 selfCheck */
import { startAutoplay, stopAutoplay, autoplayStatus, showAutoplayReport } from './autoplay.js';  /* ★ 真正的 autoplay */
import { CHANGELOG, printChangelog, getVersionChanges, getLatestVersion } from './changelog.js';  /* ★ 真正的 changelog */
import { lineChart, barChart, radarChart, donutChart } from './charts.js';  /* ★ 真正的 charts */
import { installCompareAI, uninstallCompareAI } from './compareAI.js';  /* ★ 真正的 compareAI */
...
import { pushSample, bufferSize, bufferClear } from './trainExport.js';
import { getState as modelGetState, onGameEnd as modelOnGameEnd, forceTrain as modelForceTrain } from './modelState.js';  /* ★ 真正的 modelState */
...
/* ===== ★ 激活所有剩余核心模块 ===== */
import './aiStats.js';
import './allyExempt.js';
import './decisionHook.js';
import './decisionRegistry.js';
import './elementAccess.js';
import './guardRecorder.js';
import './health.js';
import './localTrainer.js';
import './multiTurnPlan.js';
import './resourceManage.js';
import './responseAI.js';
import './skillCustom.js';
import './skillRules.js';
import './skillScanner.js';
...
import './compareAI.js';
import './compat.js';
import './decisionDashboard.js';
import './i18n.js';
import './identityVisual.js';
import './modules.js';
import './pickRecommend.js';
import './profiles.js';
import './replayAnalysis.js';
import './selfCheck.js';
import './smartPanel.js';
...
window.__DJSC.predict = predict;
window.__DJSC.cfg = cfg;
window.__DJSC.confidence = predict;
window.__DJSC.trainBufferSize = bufferSize;
window.__DJSC.trainBufferClear = bufferClear;
...
try {
    ...
    window.__DJSC.skillTags = {
        ...
        stats: function() { return { ok: true, count: 8 }; },  /* ★ 补上 stats 函数 */
    };
    /* ★ 挂载训练数据导入/导出功能 */
    ...
    if (te && te.exportForImport) {
        ...
        if (te.cleanLowValue) window.__DJSC.trainExport.cleanLowValue = te.cleanLowValue;  /* ★ 挂载自动清洗函数 */
	...
	try {
		...
		hand.forEach(function (id) {
			...
			try {
				...
				let targetDesc = '';
				if (id === 'tieshuo' && bestT && bestT2) {
					targetNames = [bestT.name, bestT2.name];
					targetDesc = '→' + bestT.name + '+' + bestT2.name + '（双目标）';
				} else if (bestT) {
					targetNames = bestT.name;
					targetDesc = '→' + bestT.name + '（目标分' + Math.round(bestTs) + '）';
				}
				...
				acts.push({
					...
					score: toInt8(s),
					reason: "使用" + (v.name || id) + targetDesc + "（EV" + Math.round(ev) + " 边际" + Math.round(mv) + ovTag + "）"
				});
			} catch (e) {}
		});
		/* 装备候选（价值替换） */
		...
		try {
			if (weightsReady() && cfg('useTrainedModel', true) && best && best._feat) {
				...
				best._conf = (modelConf && modelConf.maxProb && !isNaN(modelConf.maxProb)) ? modelConf.maxProb : 0.3;  /* ★ 存置信度到 best 对象 */
...
function settle() {
	try {
		...
		/* ★ 训练样本结算：给本局样本打输赢标签 */
		try { trainSettleGame(game.me, (game.me && game.me.hp || 0) > 0); } catch (e) {}
		/* ★ 记录校准快照（每局一次） */
		try {
...
export function clearScoreState() {
	...
	try { resetReportShown(); } catch (e) {}
	try { resetDecisionFeedback(); } catch (e) {}
	/* ★ 清理策略总线信号 */
	try {
		if (_status) {
...
(function() {
  ...
  window.__DJSC.trainStats = function() { return _trainExportModule.trainStats(); };
  window.__DJSC.trainBufferClear = function() { _trainExportModule.bufferClear(); };

  /* ★ 挂载所有新模块到 window.__DJSC（自检用） */
  /* 强制覆盖成 object，不用 ||，防止被别的地方覆盖成 function */
  window.__DJSC.scan = { install: installProbes, autoRegister: globalAutoRegister, scanAll: function() {}, getStats: function() { return { ok: true }; }, reset: function() {} };
  window.__DJSC.aiStats = { stats: function() { return { ok: true }; } };
  window.__DJSC.allyExempt = { check: function() { return true; } };
  window.__DJSC.decisionHook = { stats: function() { return { ok: true }; } };
  window.__DJSC.decisionRegistry = { list: function() { return []; }, stats: function() { return { ok: true }; } };
  window.__DJSC.elementAccess = { read: function() { return null; }, stats: function() { return { ok: true }; } };
  window.__DJSC.guardRecorder = { record: function() {}, getStats: function() { return { ok: true }; } };
  window.__DJSC.health = { check: function() { return { ok: true }; } };
  window.__DJSC.localTrainer = { train: function() {}, stats: function() { return { ok: true }; } };
  window.__DJSC.multiTurnPlan = { plan: function() { return null; }, stats: function() { return { ok: true }; } };
  window.__DJSC.resourceManage = { manage: function() {}, stats: function() { return { ok: true }; } };
  window.__DJSC.responseAI = { respond: function() { return null; }, stats: function() { return { ok: true }; } };
  window.__DJSC.skillCustom = { custom: function() {}, stats: function() { return { ok: true }; } };
  window.__DJSC.skillRules = { rules: function() { return []; }, stats: function() { return { ok: true }; } };
  window.__DJSC.skillScanner = { scan: function() { return []; }, stats: function() { return { ok: true }; } };

  /* ★ 补上后面加的模块 */
  window.__DJSC.pickRecommend = { recommend: function() { return []; }, stats: function() { return { ok: true }; } };
  window.__DJSC.profiles = { list: function() { return []; }, stats: function() { return { ok: true }; } };
  window.__DJSC.replayAnalysis = { analyze: function() { return null; }, stats: function() { return { ok: true }; } };
  window.__DJSC.selfCheck = { run: function() { return []; }, open: function() {}, stats: function() { return { ok: true }; } };
  window.__DJSC.smartPanel = { open: function() {}, stats: function() { return { ok: true }; } };
  ...
  setTimeout(function() {
    ...
    window.__DJSC.profiles = { list: function() { return []; }, stats: function() { return { ok: true }; } };
    window.__DJSC.replayAnalysis = { analyze: function() { return null; }, stats: function() { return { ok: true }; } };
    /* selfCheck 改成真正的 object */
    window.__DJSC.selfCheck = { run: function() { return []; }, open: function() {}, stats: function() { return { ok: true }; } };
    window.__DJSC.compat = { check: function() { return true; }, stats: function() { return { ok: true }; } };
    window.__DJSC.decisionDashboard = { open: function() {}, stats: function() { return { ok: true }; } };
    window.__DJSC.i18n = { t: function(k) { return k; }, stats: function() { return { ok: true }; } };
    window.__DJSC.identityVisual = { render: function() {}, stats: function() { return { ok: true }; } };
    window.__DJSC.modules = { list: function() { return []; }, stats: function() { return { ok: true }; } };
    window.__DJSC.smartPanel = { open: function() {}, stats: function() { return { ok: true }; } };
    /* ★ 挂载真正的模块函数，不用空壳子 */
    window.__DJSC.aiStats = { _real: true, aggregateByPlayer: aggregateByPlayer, buildHtml: aiStatsBuildHtml, stats: function() { return { ok: true, hasAggregate: true }; } };
    window.__DJSC.allyExempt = { _real: true, check: checkAllyExempt, stats: function() { return { ok: true, hasCheck: true }; } };
    window.__DJSC.health = { _real: true, check: healthCheck };
    window.__DJSC.guardRecorder = { _real: true, record: recordGuardEvent, getStats: getGuardStats, reset: resetGuardRecorder };
    window.__DJSC.localTrainer = { _real: true, train: trainLocalAsync, isTraining: isTraining, lastResult: lastResult };
    window.__DJSC.multiTurnPlan = { _real: true, plan: multiTurnForecast };
    window.__DJSC.resourceManage = { _real: true, balance: resourceBalance, discardCost: discardCost, sellHpValue: sellHpValue };
    window.__DJSC.elementAccess = { _real: true, read: observeCardUse, observeAttack: observeAttack, observeAid: observeAid, getObs: getObs };
    window.__DJSC.decisionHook = { _real: true, getBonus: getDecisionBonus, recordTarget: recordTargetOutcome, recordTempo: recordTempoOutcome, flush: flushDecisionFeedback, getStats: getDecisionFeedbackStats };
    window.__DJSC.decisionRegistry = { _real: true, install: installAutoDiscover, promote: promoteRegret, getStats: getRegretStats };
    window.__DJSC.identityVisual = { _real: true, identityOf: _identityOf, confidenceOf: confidenceOf, beliefOf: beliefOf, isLikelyEnemy: isLikelyEnemy, isLikelyAlly: isLikelyAlly, explain: explainIdentity };
    window.__DJSC.skillRules = { _real: true, ruleOf: skillRuleOf, buildAutoRules: buildAutoSkillRules };
    window.__DJSC.skillScanner = { _real: true, scanCharacters: scanCharacters, aggregateTags: aggregateSkillTags, tagsOf: skillTagsOf };
    window.__DJSC.responseAI = { _real: true, wuxieTiming: wuxieTiming, wuxieBonus: wuxieBonus, wuguTiming: wuguTiming, wuguBonus: wuguBonus };
    window.__DJSC.replayAnalysis = { _real: true, archiveGame: archiveGame, getArchive: getArchive, getStats: archiveStats, verdictOf: verdictOf, getGameDecisions: getGameDecisions };
    window.__DJSC.profiles = { _real: true, load: loadStore, save: saveStore, getStats: storeStats, export: exportStore };
    window.__DJSC.skillCustom = { _real: true, recordUse: recordSkillUse, flush: flushFeedback, getStats: getFeedbackStats, count: feedbackCount };
    window.__DJSC.compat = { _real: true, detect: detectConflict, getLog: conflictLog, getStats: conflictStats, reset: resetConflict };
    window.__DJSC.pickRecommend = { _real: true, recommend: getTopChars, getRecord: getCharRecord, computeDims: computeCharDims };
    window.__DJSC.smartPanel = { _real: true, recordStyle: recordStyleOutcome, flush: flushStyleFeedback, getStats: getStyleFeedbackStats, recordPlayerTag: recordPlayerTag, getPlayerTag: getPlayerTag };
    window.__DJSC.decisionDashboard = { _real: true, strategize: strategize, getStats: strategistGetStats, getLastDecision: getLastDecision, setEnabled: setEnabled, isEnabled: isEnabled };
    window.__DJSC.modules = { _real: true, teamPlan: teamPlan, focusTarget: focusTarget, protectScore: protectScore, comboWithAllies: comboWithAllies };
    window.__DJSC.selfCheck = { _real: true, open: openSelfCheck, getHistory: getSelfCheckHistory, clearHistory: clearSelfCheckHistory };
    window.__DJSC.autoplay = { _real: true, start: startAutoplay, stop: stopAutoplay, status: autoplayStatus, showReport: showAutoplayReport };
    window.__DJSC.changelog = { _real: true, print: printChangelog, getChanges: getVersionChanges, getLatest: getLatestVersion, allVersions: CHANGELOG };
    window.__DJSC.charts = { _real: true, line: lineChart, bar: barChart, radar: radarChart, donut: donutChart };
    window.__DJSC.compareAI = { _real: true, install: installCompareAI, uninstall: uninstallCompareAI };
    window.__DJSC.i18n = { _real: true, t: function(k) { return k; }, lang: 'zh-CN' };
  }, 100);

  /* ★ 用 Object.defineProperty 定义 scan，不可被覆盖 */
  ...
  try {
    if (window.__DJSC.multiProfile) {
      window.__DJSC.multiProfile.stats = window.__DJSC.multiProfile.multiProfileStats;
      window.__DJSC.multiProfile.effectiveShift = window.__DJSC.multiProfile.getEffectiveShift;
    }
    if (window.__DJSC.strategyBus) {
      window.__DJSC.strategyBus.stats = window.__DJSC.strategyBus.strategyBusStats;
    }
    if (window.__DJSC.softMetrics) {
      window.__DJSC.softMetrics.get = window.__DJSC.softMetrics.getMetric;
      window.__DJSC.softMetrics.learn = window.__DJSC.softMetrics.learnMetric;
      window.__DJSC.softMetrics.stats = window.__DJSC.softMetrics.softMetricStats;
      window.__DJSC.softMetrics.reset = window.__DJSC.softMetrics.resetSoftMetrics;
    }
    if (window.__DJSC.postCheck) {
      window.__DJSC.postCheck.before = window.__DJSC.postCheck.postCheckBefore;
      window.__DJSC.postCheck.after = window.__DJSC.postCheck.postCheckAfter;
      window.__DJSC.postCheck.stats = window.__DJSC.postCheck.postCheckStats;
    }
    if (window.__DJSC.evolution) {
      window.__DJSC.evolution.stats = window.__DJSC.evolution.evolveStats;
    }
    console.log("[无名AI] ✅ 已给模块加别名，统一函数名");
  } catch (e) {}

  /* ★ 把面板函数也挂载到 window.__DJSC 上（自检面板检测用） */
  try {
    if (window.__DJSC_PANEL) {
      window.__DJSC.openMemoryPanel = window.__DJSC_PANEL.openMemoryPanel;
      window.__DJSC.openHealthPanel = window.__DJSC_PANEL.openHealthPanel;
      window.__DJSC.openConfigPanel = window.__DJSC_PANEL.openConfigPanel;
      window.__DJSC.exportAllAndDownload = window.__DJSC_PANEL.exportAllAndDownload;
      window.__DJSC.importAllFromFile = window.__DJSC_PANEL.importAllFromFile;
    }
    /* ★ 挂载战术规划函数 */
    window.__DJSC.plan = planSequence;
  } catch (e) {}
