/* ================= 决策积分引擎 · 原生 AI 接管层 =================
 * 策略：只用无名杀官方 AI 接口（mod.aiOrder + ai.effect）
 *      把 bestAction 的结论翻译成原生 AI 能理解的分数修正。
 * 不接管 chooseToUse / chooseToRespond，不破坏响应/弃牌/拼点 AI。
 * 开关：lib.config["extension_无名AI_hardOverride"]（默认 false）
 */
...
function _getBA(player) {
	try {
		...
		if (cfg('hardOverride', false) === false) return null;
		 ...
		 *   原来的问题：mod.aiOrder 和 ai.effect 会各自调用 bestAction，
		 *   导致同一个决策被记录 N 次（N 与 aiOrder/ai.effect 被调用次数相关）。
		 *   解法：用 __logged_<key> 标记，同一 key 只记录一次。 */
		try {
...
export function installAIOverride() {
	...
	try {
				...
				effect: {
					player(card, player, target) {
						try {
							...
							/* 敌方 + 攻击：不干预。让本体 ai.effect 决定 baseline（通常正值），
							 * 引擎选中的目标已经在上方 +0.6 形成相对优势；
							 * 若这里回 -0.25，会让 AI 对所有非引擎目标也降低出牌意愿，
							 * 整体表现比不接管还保守。 */
						} catch (e) { return; }
					},
				},
		...
		_installed = true;
		try { if (game.log) game.log('决策积分引擎：原生 AI 接管层已安装'); } catch (e) {}
	} catch (e) {
