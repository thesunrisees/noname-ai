...
import { VAL_CARD, VAL_EFFECT } from '../modules/misc/value-tables.js';
import { codeGainOf, skillRuleOf, detectCombo, skillProfileOf, skillBranchesOf, checkBranch, skillStagesOf, skillInteractionOf, skillTagsOf } from '../modules/feature/skills.js';
import { enemiesOf, isEnemyOf, situationFactor, targetScore, probHasBagua, probHasShan, hasVengeanceSkill, cardValueOf, clearThreatCache, seatPressure, forecastSummary, burstThreatOf, maxBurstThreat } from '../modules/misc/threat.js';
import { miniPredict, cardIdOf, MINI_W } from '../mini-model.js';
import { miniFeatures } from '../modules/feature/skills.js';
import { MEM, memLoad, memSave, memReset, memRecordAtk, memRecordHit, memKeyOf } from '../modules/misc/mem.js';
import { observeAttack, observeAid, observeCardUse, resetObs, getObs, fireAttackExpectedHit } from '../modules/feature/observer.js';
import '../modules/misc/scoreSelfMod.js';  // ★ 积分自修改器：AI 直接修改规则积分
import { deckConsume, deckReset, cardRemaining, deckAutoDetect, deckSyncFromUI } from '../modules/feature/deckMemory.js';
import { baguaSuccessRate } from '../modules/feature/deckPredict.js';
import { identityOf as _identityOf, beliefOf, updateBelief, confidenceOf, isLikelyEnemy, isLikelyAlly, resetBelief, explainIdentity } from '../modules/misc/identity.js';
import { cfg, safe, nameOf, keyOf, isObj } from '../modules/misc/util.js';
import { teamPlan } from '../modules/misc/team.js';
import { log } from '../modules/misc/logger.js';
import { getDecisionBonus, recordTargetOutcome, recordTempoOutcome, recordKeepOutcome, flushDecisionFeedback } from '../panels/decisionFeedback.js';
import { perfMark } from '../modules/misc/perf.js';
import { resourceBalance, discardCost, sellHpValue, equipReplaceCost } from '../modules/misc/economy.js';
import { styleOf } from '../modules/feature/observer.js';
import { loadStore, saveStore, mergeOnSettle, storeStats } from '../modules/feature/memory.js';
import { archiveGame, verdictOf, loadArchive, getArchive, getGameDecisions, exportArchiveJson } from '../modules/misc/archive.js';
import { trainStartGame, trainRecordSample, trainSettleGame } from '../trainExport.js';
import { loadFeedback, recordSkillUse, flushFeedback, feedbackCount } from '../modules/misc/feedback.js';
import { multiTurnForecast } from '../modules/misc/multiturn.js';
import { loadStyleFeedback, recordStyleOutcome, flushStyleFeedback, recordPlayerTag, saveStyleFeedback } from '../panels/styleFeedback.js';
import { focusBonus, broadcastIntent, installBroadcastHooks } from '../modules/misc/teamBroadcast.js';
import { refineBestWithPlan, planSequence } from '../modules/strategy/planner.js';
import { strategize } from '../modules/misc/strategist.js';
import { getModeStrategy, isSameCamp, isEnemy, applyModeBoost } from '../modules/strategy/modeStrategy.js';
/* ===== 24 个优化模块 ===== */
import { probHasWuxie, probHasTao, probHasSha, probHasJiu } from '../modules/misc/handInference.js';
import { keepScore, recommendKeep } from '../modules/strategy/keepStrategy.js';
import { equipScarcity, equipValue, isKeyEquip } from '../modules/misc/equipScarcity.js';
import { analyzeTeammateIntent, analyzeTeammateStrategy, teammateCoordination, recordTeammateAction } from '../modules/misc/teammateIntent.js';
import { analyzeOpponentPref, counterStrategy, predictOpponentNext, counterScoreBonus } from '../modules/strategy/counterStrategy.js';
import { evaluateSituation, describeSituation, situationStrategyBonus } from '../modules/misc/situationEval.js';
import { shaHitRate, wuxieRisk, duelWinRate, taoNecessity, riskScore } from '../modules/misc/riskQuant.js';
import { analyzeOpponentMood, describeMood, moodStrategyBonus, getAllMoods } from '../modules/misc/moodState.js';
import { getMutualRelations, isCountering, isBeingCountered, counterRelationBonus } from '../modules/misc/mutualRelations.js';
import { getCardPriority, recommendOrder, orderBonus, recommendFlow } from '../modules/strategy/orderOptimizer.js';
import { shouldPassCard, whatTeammateNeeds, passCardBonus } from '../modules/strategy/passStrategy.js';
import { countTiesuo, tiesuoTransfer, tianxiangTransfer, damageTransfer, damageTransferBonus } from '../modules/misc/damageTransfer.js';
import { predictShaFollowup, predictJuedouFollowup, searchTree, treeSearchBonus } from '../modules/misc/treeSearch.js';
import { needLongDelay, getSmartDelay, waitForSkills, checkSkillTriggered } from '../modules/timing/delayOptimizer.js';
import { getDeckTop, hasGuanxingSkill, prioritizeDeckTop, deckTopBonus, clearDeckTopCache } from '../modules/feature/deckTopPredict.js';
import { recommendDiscard, discardValue, specialDiscardAdvice, discardAdvice } from '../modules/strategy/discardStrategy.js';
import { recordGameResult, getWinRate, autoAdjustWeights, learningPanelData, clearLearningData } from '../modules/misc/learningLoop.js';
import { aliveCount, identityGameStrategy, shouldRevealIdentity, gameTheoryBonus } from '../modules/misc/gameTheory.js';
import { cached, clearAllCache, cacheStats, perfStart, perfEnd } from '../modules/strategy/perfOptimizer.js';
/* ===== v1.7.0 新增 5 个优化模块 ===== */
import { responseBonus } from '../modules/strategy/responseOpt.js';
import { discardBonus } from '../modules/strategy/discardOpt.js';
import { endgameBonus, isEndgame, endgameStrategy } from '../modules/strategy/endgameOpt.js';
import { opponentPredictBonus, recordOpponentAction } from '../modules/misc/opponentPredict.js';
import { resourceTimingBonus } from '../modules/timing/resourceTiming.js';
/* ===== v1.8.0 新增 5 个优化模块 ===== */
import { aoeBonus } from '../modules/timing/aoeTiming.js';
import { judgeBonus } from '../modules/timing/judgeTiming.js';
import { equipReplaceBonus } from '../modules/misc/equipReplace.js';
import { keepBonus } from '../modules/strategy/keepStrategyOpt.js';
import { multiTurnBonus } from '../modules/strategy/multiTurnOpt.js';
/* ===== v1.0.2 新增 5 个锦囊时机优化模块 ===== */
import { duelBonus } from '../modules/timing/duelTiming.js';
import { jiedaoBonus } from '../modules/timing/jiedaoTiming.js';
import { shandianBonus } from '../modules/timing/shandianTiming.js';
import { taoyuanBonus } from '../modules/timing/taoyuanTiming.js';
import { wuguBonus } from '../modules/timing/wuguTiming.js';
/* ===== v1.0.3 新增 5 个时机优化模块 ===== */
import { shaTargetBonus } from '../modules/strategy/shaTargetOpt.js';
import { taoBonus } from '../modules/timing/taoTiming.js';
import { jiuBonus } from '../modules/timing/jiuTiming.js';
import { wuxieBonus } from '../modules/timing/wuxieTiming.js';
import { shunshouBonus } from '../modules/timing/shunshouTiming.js';
/* ===== v1.0.4 深度价值量化模块 ===== */
import { deepValueBonus, deepCardValue, deepTargetValue, deepSituationValue } from '../modules/misc/deepValue.js';
import { recordTrigger, getDecayMultiplier, applyDecay, clearDecayLog, getDecayStats } from '../modules/strategy/decayOpt.js';
import { toInt8, toFloat, unbiasedRound, clearCompensation } from '../modules/misc/scoreUnify.js';
...
import { observeElementUse, startElementFeedbackLoop, settleElementFeedback } from '../panels/elementFeedback.js';
import { metaStartGame, metaSettleGame, metaRecordSkill, metaRecordCard, metaRecordTarget, metaRecordDecision, cognitiveModulate, decideIntervention } from '../modules/misc/metaCognition.js';
import '../panels/cognitiveLog.js';
import '../modules/misc/conflictDetector.js';
import '../modules/decision/decisionCalibrator.js';
import '../panels/calibratorPanel.js';
import '../modules/misc/multiProfile.js';
import '../modules/strategy/strategyBus.js';
import '../panels/brainDashboard.js';
import '../modules/decision/decisionReplay.js';
import '../exportAll.js';
import '../panels/replayPanel.js';
import '../modules/misc/weightPersist.js';
import '../modules/misc/crossModeTransfer.js';
import '../modules/decision/decisionCompare.js';
import '../panels/comparePanel.js';
import '../modelHotSwap.js';
import '../modules/misc/sharedKnowledge.js';
import '../modules/strategy/evolution.js';
import { psychologyBonus, deterrenceCheck, intentReading, pressureScore, strategicHold, psychologyStats, resetPsychology } from '../modules/misc/psychology.js';
import { comboChainBonus, detectChains, chainScore, chainPriority, comboChainStats, resetComboChain, CHAINS } from '../modules/ai/comboChain.js';
import { playerMemoryBonus, rememberGame, rememberAttack, rememberAid, playerMemoryStats, recallPlayer, hostilityLevel, playerMemoryList, resetPlayerMemory } from '../modules/feature/playerMemory.js';
import { profStart, profEnd, profile, profilerEnable, profilerStats, openProfilerPanel } from '../modules/misc/profiler.js';
import { narrate, renderNarrateHtml, recentNarrations, showRecentNarrations } from '../modules/decision/decisionNarrator.js';
import { postCheckBefore, postCheckAfter, postCheckDelayed, postCheckStats, postCheckReset } from '../modules/misc/postCheck.js';
import { getCardStrategy, cardUseValue, cardRespondValue, getCardType, getCardRisk, cardStrategyStats, resetCardStrategy } from '../modules/strategy/cardStrategy.js';
import { recordDecisionContext, settleDecisionContext, getAutoFeatureWeight, autoFeatureStats, topAutoFeatures, resetAutoFeatures } from '../modules/feature/autoFeature.js';
import { getMetric, learnMetric, learnFromGame, softMetricStats, resetSoftMetrics } from '../modules/misc/softMetrics.js';

/* ★ v2.2.5 ~ v2.3.2 新模块 import（必须 import 才会执行挂载） */
import '../modules/feature/skillTags.js';      // 技能标签系统
import '../modules/misc/judgeZone.js';      // 判定区状态检测
import '../modules/feature/cardTags.js';       // 手牌标记系统
import '../modules/misc/viewAs.js';         // viewAs/转化类AI
import '../modules/misc/costCalc.js';       // cost函数计算
import '../modules/ai/aiTools.js';        // AI工具集（嘲讽度/技能重要度/技能标签/回合外价值）
import '../modules/strategy/learningOptimizer.js';  // 学习效率优化器（优先级回放/特征筛选/课程学习/自适应学习率）
import '../modules/timing/skillTiming.js';       // 技能触发时机识别（触发时机/技能类型/效果识别）
import { openSmartPanel } from '../panels/smartPanel.js';
import { openReplayPanel } from '../panels/replayPanel.js';
import { openDecisionDashboard } from '../panels/decisionDashboard.js';
import { openComparePanel } from '../panels/comparePanel.js';
import { openCalibratorPanel } from '../panels/calibratorPanel.js';
import { openBrainDashboard } from '../panels/brainDashboard.js';
...
function installHooks() {
	...
	try { import('../modules/misc/selfHeal.js').then(function (m) { m.autoSelfHeal(3000); }).catch(function () {}); } catch (e) {}
...
function settle() {
	try {
		...
		/* ★ 自适应难度 */
		try { import('../modules/misc/adaptive.js').then(function (m) { m.updateAdaptive && m.updateAdaptive(); }).catch(function () {}); } catch (e) {}
		/* 延迟弹出战报（避免与本体结算界面冲突） */
		try {
			setTimeout(function () {
				try {
					import('../modules/misc/report.js').then(function (m) {
		...
		/* ★ Bandit：本局结果回填给决策点 */
		try {
			import('../modules/ai/bandit.js').then(function (m) {
		...
		/* ★ 自动发现：检查后悔值，注册新决策点 */
		try {
			import('../modules/misc/autoDiscover.js').then(function (m) {
				m.promoteHighRegretPoints();
			});
		} catch (eDiscover) {}
		/* ★ 全局扫描：每局结束时检查新决策点 */
		try {
			import('../modules/misc/globalScanner.js').then(function (m) {
				m.autoRegister();
			});
		} catch (eScan) {}
	} catch (e) {}
...
export function clearScoreState() {
	...
	try { import('../modules/misc/report.js').then(function (m) { m.resetReportShown && m.resetReportShown(); }); } catch (e) {}
	try { import('../panels/decisionFeedback.js').then(function (m) { m.resetDecisionFeedback && m.resetDecisionFeedback(); }); } catch (e) {}
	/* ★ 清理策略总线信号 */
	try {
		if (_status) {
...
export { loadStore, saveStore, storeStats } from '../modules/feature/memory.js';
...
try {
    ...
    /* 模型权重 */
    window.__DJSC.weightsReady = window.__DJSC.weightsReady || function () { return false; };
    window.__DJSC.predict = window.__DJSC.predict || function () { return null; };
    window.__DJSC.reloadWeights = window.__DJSC.reloadWeights || function () { return false; };
    
    /* 训练数据 */
    window.__DJSC.trainBufferSize = window.__DJSC.trainBufferSize || function () { return 0; };
    window.__DJSC.trainBufferClear = window.__DJSC.trainBufferClear || function () {};
    window.__DJSC.trainLocalAsync = window.__DJSC.trainLocalAsync || function () { return Promise.resolve(); };
    
    /* 模型状态 */
    window.__DJSC.modelState = window.__DJSC.modelState || {};
    window.__DJSC.modelState.onGameEnd = window.__DJSC.modelState.onGameEnd || function () {};
    window.__DJSC.modelState.forceTrain = window.__DJSC.modelState.forceTrain || function () {};
    
    /* 决策注册 */
    window.__DJSC.decision = window.__DJSC.decision || {};
    window.__DJSC.decision.listDecisionPoints = window.__DJSC.decision.listDecisionPoints || function () { return []; };
    window.__DJSC.decision.setTrust = window.__DJSC.decision.setTrust || function () {};
    
    /* Bandit */
    window.__DJSC.bandit = window.__DJSC.bandit || {};
    window.__DJSC.bandit.getStats = window.__DJSC.bandit.getStats || function () { return {}; };
    window.__DJSC.bandit.updateTrust = window.__DJSC.bandit.updateTrust || function () {};
    
    /* 全局扫描 */
    window.__DJSC.scan = window.__DJSC.scan || {};
    window.__DJSC.scan.install = window.__DJSC.scan.install || function () {};
    window.__DJSC.scan.autoRegister = window.__DJSC.scan.autoRegister || function () {};
    
    /* 自动发现 */
    window.__DJSC.discover = window.__DJSC.discover || {};
    window.__DJSC.discover.getStats = window.__DJSC.discover.getStats || function () { return {}; };
    
    /* 软接管 */
    window.__DJSC.aiOverride = window.__DJSC.aiOverride || {};
    
    /* 面板 */
    window.__DJSC.openPanel = openSmartPanel;
    window.__DJSC.openScorePanel = openSmartPanel;
    window.__DJSC.openPlanPanel = window.__DJSC.openPlanPanel || function () { alert('战术规划面板开发中...'); };
    window.__DJSC.openFeedbackPanel = openReplayPanel;
    window.__DJSC.openArchivePanel = window.__DJSC.openArchivePanel || function () { alert('战报归档面板开发中...'); };
    window.__DJSC.openRecommendPanel = window.__DJSC.openRecommendPanel || function () { alert('选将推荐面板开发中...'); };
    window.__DJSC.openSkillPanel = window.__DJSC.openSkillPanel || function () { alert('技能矩阵面板开发中...'); };
    window.__DJSC.openOverridePanel = window.__DJSC.openOverridePanel || function () { alert('接管面板开发中...'); };
    window.__DJSC.openMemoryPanel = window.__DJSC.openMemoryPanel || function () { alert('记忆面板开发中...'); };
    window.__DJSC.openHealthPanel = window.__DJSC.openHealthPanel || function () { alert('健康面板开发中...'); };
    window.__DJSC.openConfigPanel = window.__DJSC.openConfigPanel || function () { alert('配置面板开发中...'); };
    ...
    /* 数据导入导出 */
    window.__DJSC.exportAllAndDownload = window.__DJSC.exportAllAndDownload || function () { alert('数据导出开发中...'); };
    window.__DJSC.importAllFromFile = window.__DJSC.importAllFromFile || function () { alert('数据导入开发中...'); };
    
    /* 开机自修复 */
    window.__DJSC.selfHeal = window.__DJSC.selfHeal || {};
    window.__DJSC.selfHeal.run = window.__DJSC.selfHeal.run || function () {};
    window.__DJSC.selfHeal.auto = window.__DJSC.selfHeal.auto || function () {};
    
    /* 内奸判断 */
    if (!window.__DJSC.identity) window.__DJSC.identity = {};
    window.__DJSC.identity.believe = window.__DJSC.identity.believe || function () { return 0.5; };
    
    console.log('✅ 兜底接口补齐完成');
} catch (e) {
    console.warn('兜底接口补齐失败：', e);
}


/* ================= 最终覆盖：确保真实面板覆盖兜底代码 =================
 * 兜底代码在文件末尾，可能会覆盖之前的真实面板挂载
 * 这里再重新挂载一遍，确保真实面板生效
 */
try {
    window.__DJSC.openPanel = openSmartPanel;
    ...
    window.__DJSC.openCalibratorPanel = openCalibratorPanel;
    window.__DJSC.openBrainDashboard = openBrainDashboard;
    console.log('✅ 面板最终挂载完成');
} catch (e) {
    console.warn('面板最终挂载失败：', e);
}
