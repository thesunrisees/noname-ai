...
/* 导入所有模块 */
import { probHasShan, probHasWuxie, probHasTao, probHasSha, probHasJiu, inferHand } from './handInference.js';
import { keepScore, recommendKeep, suitValue, cardKeepValue } from './keepStrategy.js';
import { equipScarcity, equipValue, sortEquips, isKeyEquip } from './equipScarcity.js';
import { predictCombo, predictTaoNeed, multiTurnScore } from './multiTurnPlan.js';
import { analyzeTeammateIntent, analyzeTeammateStrategy, teammateCoordination, recordTeammateAction } from './teammateIntent.js';
import { analyzeOpponentPref, counterStrategy, predictOpponentNext } from './counterStrategy.js';
import { evaluateSituation, describeSituation, situationStrategyBonus } from './situationEval.js';
import { shaHitRate, wuxieRisk, duelWinRate, riskScore, describeRisk, taoNecessity } from './riskQuant.js';
import { taoTiming, wuxieTiming, jiuTiming, resourceScarcity } from './resourceManage.js';
import { analyzeOpponentMood, describeMood, moodStrategyBonus, getAllMoods } from './moodState.js';
import { inferIdentity, describeIdentity, getAllIdentities, identityPanelData } from './identityVisual.js';
import { recordDecision, getDecisionLog, clearDecisionLog, analyzeReplay, replayPanelData } from './replayAnalysis.js';
...
import { needLongDelay, getSmartDelay, checkSkillTriggered } from './delayOptimizer.js';
...
import { cached, clearAllCache, cacheStats, perfStart, perfEnd, perfReport } from './perfOptimizer.js';
...
export function openSmartPanel() {
    const active = _status.currentPhase || game.me;
    const html = active ? generateSmartPanelHtml() : '<p>当前没有进行中的对局。开始对局后，这里会显示局面、风险和学习数据。</p>';
    openUtilityHtml('无名AI · 智能可视化', html, 'djsc-smart-panel');
}
