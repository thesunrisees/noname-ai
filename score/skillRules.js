...
export function renderScoreBreakdown(sid, tags, ctx) {
    try {
        ...
        L.push('');
        L.push('【八维度】');
        L.push('  对象：' + ({self:'自己', team:'队友', enemy:'敌人', multiEnemy:'多敌', multiTeam:'多友', all:'全场', dying:'濒死'}[d.object] || d.object) + ' × ' + OBJECT_MUL[d.object]);
        L.push('  范围：' + ({single:'单体', few:'少量', many:'多体', all:'全体'}[d.range] || d.range) + ' × ' + RANGE_MUL[d.range]);
        L.push('  时机：' + ({dying:'濒死', damageAfter:'伤害后', damaged:'受伤后', chooseToRespond:'响应选择', phaseUse:'出牌阶段', phaseDraw:'摸牌阶段', useCard:'使用牌', chooseToUse:'使用选择', phaseZhunbei:'准备阶段', phaseDiscard:'弃牌阶段', phaseJieshu:'结束阶段', judge:'判定阶段', passive:'被动', die:'死亡'}[d.timing] || d.timing) + ' × ' + TIMING_MUL[d.timing]);
        L.push('  频率：' + ({limit:'限定技', awaken:'觉醒技', perRound:'每轮', rare:'稀有', perTurn:'每回合', frequent:'频繁', passive:'被动', locked:'锁定'}[d.frequency] || d.frequency) + ' × ' + FREQ_MUL[d.frequency]);
        L.push('  风险：' + ({none:'无', judge:'判定', compare:'拼点', chance:'概率'}[d.risk] || d.risk) + ' × ' + RISK_MUL[d.risk]);
        L.push('  持续：' + ({instant:'即时', turn:'一轮', round:'一回合', game:'整局', forever:'永久'}[d.duration] || d.duration) + ' × ' + DURATION_MUL[d.duration]);
        L.push('');
