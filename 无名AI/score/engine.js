...
import { identityOf as _identityOf, beliefOf, resetBelief } from './identity.js';
...
function give(char, pts, tag) {
	try {
		if (!char) return;
		const k = keyOf(char);
		round[k] = Math.round(((round[k] || 0) + pts) * 100) / 100;
		scoreLog.push({ char: k, pts: Math.round(pts * 100) / 100, tag: tag, ts: Date.now() });
		if (scoreLog.length > 400) scoreLog.shift();
		/* 不再每条记分都 game.log（会刷屏）。
		 * 记分只累积到 round/scoreLog，供面板与结算汇总使用；
		 * 终局结算时统一打印一次带阵营标注的汇总。 */
	} catch (e) {}
}
...
function scoreCardUse(me, card, target) {
	try {
		...
		if (target && isObj(target)) {
			...
			if (isAlly) {
				/* ★ 锦囊类特殊处理：乐/兵/铁索横置 → 直接重罚 */
				const PENALTY_CARDS = ['lebu', 'bingliang', 'tiesuo', 'shunshou'];
				if (PENALTY_CARDS.indexOf(id) >= 0) {
					/* 判断是横置还是解除横置 */
					if (id === 'tiesuo') {
						/* 铁索连环：横置重罚，解除横置加分 */
						const isLinking = !target.isLinked; /* 假设 target.isLinked 表示是否已横置 */
						if (isLinking) {
							/* 横置友方 → 重罚 */
							give(me, -Math.abs(v.use) * 3, "对友方铁索横置（严重违规）");
							giveVs(me, -Math.abs(v.use), "阵营分扣除");
							return;
						} else {
							/* 解除友方横置 → 加分 */
						...
						if (isJudgeArea) {
							...
							/* 拿非判定区 → 重罚 */
							give(me, -Math.abs(v.use) * 3, "顺友方非判定区（严重违规）");
							giveVs(me, -Math.abs(v.use), "阵营分扣除");
							return;
						}
					} else {
						/* 乐不思蜀 / 兵粮寸断 → 重罚 */
						give(me, -Math.abs(v.use) * 3, "对友方使用" + (id === 'lebu' ? '乐不思蜀' : '兵粮寸断') + "（严重违规）");
						giveVs(me, -Math.abs(v.use), "阵营分扣除");
						return;
					}
				}
			...
			target.forEach(function (t) {
				...
				let isAlly = false;
				try {
					if (get.attitude(me, t) > 0) isAlly = true;
					const strategy = typeof getModeStrategy === 'function' ? getModeStrategy() : null;
					if (strategy && strategy.getCamp) {
...
const EFFECT_HANDLERS = {
	damage: function (me, a) {
		...
		if (src && src !== me) {
			...
			let isAlly = false;
			try {
				if (get.attitude(src, me) > 0) isAlly = true;
			...
			try {
				...
				try {
					...
					if (killer) {
						...
						let isAlly = false;
						try {
							if (get.attitude(killer, me) > 0) isAlly = true;
							const strategy = getModeStrategy();
...
function marginalValue(id, handIds) {
	try {
		...
		return Math.round(v.use * Math.pow(0.6, count) * 100) / 100;
	} catch (e) { return 0; }
}
	...
	try {
			...
			try {
				...
				try {
					...
					if (bestT && DMG.indexOf(id) >= 0) {
						const att = get.attitude(me, bestT);
						if (att > 0) {
						...
						if (dyingAlly) {
							...
							try {
								for (const p of (game.players || [])) {
									...
									if (get.attitude(me, p) >= 0) continue;
									let knownTao = 0;
									try {
				...
				if (id === 'jiu') {
					try {
						...
						for (const p of (game.players || [])) {
							...
							if (get.attitude(me, p) > 0) { dyingAlly = p; break; }
						}
						if (dyingAlly) {
							const hpDeficit = Math.max(1, -(dyingAlly.hp || 0) + 1);
				...
				if (id === 'nanman' || id === 'wanjian') {
					try {
						...
						for (const p of (game.players || [])) {
							...
							if (get.attitude(me, p) > 0) allyAtRisk += w;
							else enemyAtRisk += w;
						}
						/* 自身：若自己无防御牌也计入风险 */
							...
							try {
								...
								if (alive <= 4) {
									...
									for (const p of (game.players || [])) {
										...
										if (get.attitude(me, p) > 0) allyLowHp++;
										else enemyLowHp++;
									}
				...
				try {
					...
					if (hasZhuge && myShaCount > 0) {
						...
						if (id === 'guohe' || id === 'shunshou') {
							if (bestT && get.attitude(me, bestT) < 0) {
								/* 目标有装备 → 强前置 */
				...
				if (bestT) {
					...
					try {
						...
						if (!isAlly) {
							isAlly = (get.attitude(me, bestT) > 0);
						}
					} catch(e) {
						isAlly = (get.attitude(me, bestT) > 0);
					}
				...
				acts.push({ type: "card", id: id, target: bestT ? bestT.name : null, targetScore: bestTs, score: Math.round(s * 100) / 100, reason: "使用" + (v.name || id) + (bestT ? "→" + bestT.name + "（目标分" + bestTs + "）" : "") + "（EV" + ev + " 边际" + mv + ovTag + "）" });
			} catch (e) {}
		});
		/* 装备候选（价值替换） */
		try {
		...
		try {
			...
			try {
				...
				for (const p of (game.players || [])) {
					if (!p || p === me || p.alive === false) continue;
					if ((p.hp || 0) <= 0 && get.attitude(me, p) > 0) { dyingAlly = true; break; }
				}
...
function settle() {
	try {
		...
		try {
			...
			/* ★ 训练数据：回填 reward */
			try { trainSettleGame(myScore, me ? me.identity : ''); } catch (e) {}
...
export function clearScoreState() {
	settleDone = false; round = {}; scoreLog = [];
