			...
			if (!logs || !logs.length) {
				...
				html += "<div class='djsc-replay-card' data-gap='" + Math.round(gapVal * 100) / 100 + "' style='margin:8px 0;padding:6px 8px;border-left:3px solid #5a7aa8;background:rgba(255,255,255,0.02);'>";
					...
					if (entry.candidates && entry.candidates.length) {
						...
						const maxAbs = Math.max(1, Math.max.apply(null, cands.map(function (c) { return Math.abs(c.score || 0); })));
						...
						["skill", "card", "equip", "other"].forEach(function (g) {
							...
							const maxS = Math.max.apply(null, groups[g].map(function (c) { return Math.abs(c.score || 0); })) || 1;
							const sumS = groups[g].reduce(function (s, c) { return s + (c.score || 0); }, 0);
							...
							groups[g].slice(0, 3).forEach(function (c) {
								const pct = Math.round((Math.abs(c.score || 0) / maxS) * 100);
								const isWin = w && c.type === w.type && c.id === w.id;
				...
				if (!plan) {
					...
					if (plan.isKill) {
						...
						(plan.all || []).slice(0, 5).forEach(function (s, i) {
							...
							html += "<span style='display:inline-block;width:56px'>" + (s.baseScore || 0) + "</span>";
							html += "<span style='display:inline-block;width:56px;color:#ffd479'>+" + (s.futureScore || 0) + "</span>";
							html += "<span style='color:" + col + ";font-weight:500;'>" + s.total + "</span><br>";
						});
						html += "</div>";
...
function installDebugBridge() {
	try {
		...
		window.__DJSC = {
			...
			/* 身份推理探针 */
			identity: function (p) { try { return identityOf(p || game.me); } catch (e) { return "unknown"; } },
