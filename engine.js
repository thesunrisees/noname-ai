...
function bestAction() {
	...
	try {
		...
		/* ================= ★ 第1层：违规样本检测 ================= */
		try {
			let _violationFlag = null;
			if (best && best.type === 'card' && bestT) {
				const VIOLATION_IDS = ['sha','juedou','huogong','nanman','wanjian','lebu','bingliang','shunshou','guohe','tiesuo'];
				if (VIOLATION_IDS.indexOf(best.id) >= 0) {
					let isAlly = false;
					try {
					...
					if (isAlly) {
						let whitelist = false;
						if (best.id === 'tiesuo' && bestT.isLinked && bestT.isLinked()) whitelist = true;
						if (best.id === 'shunshou' && bestT.judges && bestT.judges.length > 0) whitelist = true;
						if (!whitelist) {
							_violationFlag = {
								type: 'ally_attack',
								card: best.id,
								target: bestT.name1 || bestT.name,
								reason: (best.reason || '').slice(0, 80),
								ts: Date.now(),
							};
						}
					}
			...
			try { _status.djsc_lastViolation = _violationFlag; } catch (e) {}
		} catch (eV) {}
		...
		try {
			...
			for (let t = 0; t < times; t++) {
				trainRecordSample(me, best, sit, best.score);
			}
		} catch (e) {}
		/* ★ 调用总线仲裁 */
		try {
